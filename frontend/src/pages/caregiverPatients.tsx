...
import {
  ...
  getCaregiverAvailableSections,
  getCaregiverMessage,
  type CaregiverDetailSection,
  sortCaregiverPatientLinks,
} from "../utils/caregiver";
import "./patientHome.css";

type CaregiverPatientsPageProps = {
  apiUrl: string;
  user: User;
  onOpenHome: () => void;
...
export default function CaregiverPatientsPage({
  ...
  const [activeSection, setActiveSection] =
    useState<CaregiverDetailSection>("information");
  ...
  const availableSections = useMemo(
    () =>
      selectedLink ? getCaregiverAvailableSections(selectedLink.permissions) : [],
    [selectedLink]
  );
    ...
    const loadOverview = async () => {
      ...
      try {
        const response = await fetch(
          `${apiUrl}/api/users/${user.id}/patient-links/${selectedPatientId}/overview`
        );
        const data = (await response.json()) as {
          error?: string;
          permissions?: CaregiverPatientOverview["permissions"];
          patient?: CaregiverPatientOverview["patient"];
          dashboard?: CaregiverPatientOverview["dashboard"];
          decisions?: CaregiverPatientOverview["decisions"];
          documents?: CaregiverPatientOverview["documents"];
        };
        ...
        if (
          ...
          if (!ignore) {
            setOverview(null);
            setOverviewMessage(getCaregiverMessage(data.error));
          }
          return;
        }
